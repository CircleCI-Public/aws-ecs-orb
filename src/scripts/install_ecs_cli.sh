...
ORB_EVAL_INSTALL_DIR=$(circleci env subst "${ORB_EVAL_INSTALL_DIR}")

# Platform check
if uname -a | grep "Darwin"; then
    export SYS_ENV_PLATFORM="darwin"
    MACOS_VERSION=$(sw_vers -productVersion | cut -d. -f1)
    DELIMIT_VERSION=$(echo "$ORB_EVAL_VERSION" | cut -dv -f2)
    MAJOR=$(echo "$DELIMIT_VERSION" | cut -d. -f1)
    MINOR=$(echo "$DELIMIT_VERSION" | cut -d. -f2)
    if [ "$MACOS_VERSION" -ge "12" ] && [ "$MAJOR" -le "1" ] && [ "$MINOR" -lt "9" ]; then
        echo "Error: ECS CLI version ${ORB_EVAL_VERSION} is not supported with macOS version ${MACOS_VERSION}. Please upgrade to macOS version 1.9 or later."
        exit 1
    fi
...
if ! command -v ecs-cli; then
    ...
    Install_ECS_CLI "${ORB_EVAL_VERSION}"
    ecs-cli --version
else
    if [ "$ORB_VAL_OVERRIDE_INSTALLED" = 1 ]; then
